...
Build a governance system that is authoritative, searchable, minimally repetitive, and safe to evolve.
...
4. Put invariant rules and a task-to-section reading map at the top of the authoritative specification. Keep one H1 and a stable H2/H3 hierarchy.
5. Separate current rules, verified implementation facts, historical plans, and private experiments.
6. Maintain one status index for defects and improvements while giving each record type its own required fields. Append history, link superseded behavior, and never invent missing timestamps or evidence.
7. Add a read-only validator for structure, identifiers, fields, evidence, and time consistency. Integrate it without weakening existing protections.
8. Verify proportionally: L1 for low-risk documentation or presentation changes, L2 for one module or behavior, and L3 for a completed stage or commit boundary.
9. Report the resulting structure, checks run, unresolved items, and next precise entry point.
